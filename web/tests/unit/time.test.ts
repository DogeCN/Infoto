...
describe('announcement time formatting', () => {
  it('formats concise relative time against the supplied reference', () => {
    ...
    expect(formatRelativeTime(reference - 60_000, reference)).toBe('1 minutes ago');
    expect(formatRelativeTime(reference - 3_600_000, reference)).toBe('1 hours ago');
    expect(formatRelativeTime(reference - 86_400_000, reference)).toBe('1 days ago');
    expect(formatRelativeTime(reference - 8 * 86_400_000, reference)).toBe('8 days ago');
    expect(formatRelativeTime(reference - 45 * 86_400_000, reference)).toBe('1 months ago');
    expect(formatRelativeTime(reference - 400 * 86_400_000, reference)).toBe('1 years ago');
  });
