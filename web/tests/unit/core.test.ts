...
describe('keepalivePrefix', () => {
	...
	it('cuts the longest fitting prefix, multi-byte payloads measured exactly', () => {
		...
		const big: Op = { type: 'fb_create', target: null, payload: { contentMd: '测'.repeat(2000) + '🔥' } };
		...
		expect(bytes(`{"ops":[${JSON.stringify(like(1))},${JSON.stringify(big)},${JSON.stringify(big)}]}`)).toBeGreaterThan(8_000);
	});

	it('prefix keeps at least 1 op; a lone oversize op returns null', () => {
		const giant: Op = { type: 'ann_create', target: null, payload: { title: 't', contentMd: 'x'.repeat(70_000) } };
