...
describe('upload purpose routing', () => {
  ...
  it('resolves and rejects only an editor terminal with an owning waiter', () => {
    expect(pipelineResultAction(status('editor', 'done', { url: 'https://x' }), true)).toBe('resolve');
    expect(pipelineResultAction(status('editor', 'failed', { error: 'bad_image' }), true)).toBe('reject');
    expect(pipelineResultAction(status('editor', 'done', { url: 'https://x' }), false)).toBe('ignore');
    expect(pipelineResultAction(status('album', 'done', { url: 'https://x' }), true)).toBe('observe');
  });
