...
async function transcodeGif(jobId: string, file: Blob): Promise<VideoWorkerResult> {
	...
	for (let i = 0; i < frameCount; i++) {
		...
		const sample = new VideoSample(image, { timestamp: i * (durationUs / 1e6), duration: durationUs / 1e6 });
...
self.onmessage = async (e: MessageEvent<VideoJobMessage>) => {
	...
	try {
		const result = msg.engine === 'gif' ? await transcodeGif(msg.jobId, msg.file) : await transcodeVideo(msg.jobId, msg.file);
		// structured clone, no transfer list (a Blob never goes into transfers)
		self.postMessage({ t: 'videoResult', ...result });
	} catch (err) {
		self.postMessage({ t: 'videoFailed', jobId: msg.jobId, error: String((err as Error)?.message ?? err) });
	}
};
