...
export class LeaseClient {
	...
	constructor(private port: LeasePort, handlers: LeaseHandlers) {
	...
	release(): void {
		this.stopHeartbeat();
		if (this.leaseId) {
			this.port.postMessage({ t: 'leaseRelease', leaseId: this.leaseId } satisfies { t: 'leaseRelease'; leaseId: string });
			this.leaseId = null;
			this.jobId = null;
		}
	}
