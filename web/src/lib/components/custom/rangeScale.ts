...
export function normalizeRangeValue(
  ...
  return clampNormalized(
    (Math.log1p(Math.max(0, clamped)) - domainStart) / domainLength,
  );
}
...
export function mapRangeValue(
  ...
  const domainStart = Math.log1p(Math.max(0, min));
  const value = Math.expm1(
    domainStart + t * (Math.log1p(Math.max(0, max)) - domainStart),
  );
