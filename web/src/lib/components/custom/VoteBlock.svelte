<script lang="ts">
  ...
  // 当前用户已投的选项序号（-1 表示未投）
  let chosen = $derived(
    votes.findLast((v) => v.userId === selfId)?.option ?? -1,
  );
...
<div class="space-y-1.5">
  {#each options as label, idx (idx)}
    ...
    >
      ...
      ></span>
      <span
        class="vote-meta relative z-10 flex items-center justify-between gap-2"
      >
        <span
          class="text-sm leading-tight {isChosen || isWinner
            ? 'text-primary'
            : 'text-foreground'}"
        >
          {label}
