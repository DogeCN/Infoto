<script lang="ts">
  ...
  const canSave = $derived(title.trim().length > 0 && contentMd.trim().length > 0);
  ...
  >
      ...
      <div class="min-w-0">
        ...
        {#if announcement}
          <p class="mt-1 text-xs text-muted-foreground">
            上次更新于 {new Date(announcement.updatedAt).toLocaleString('zh-CN')}
          </p>
        {/if}
      </div>
      <button
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-4 py-4 md:px-6">
      <div class="mx-auto flex h-full max-w-6xl flex-col gap-4">
        <div>
          <label for="announcement-title" class="mb-1.5 block text-sm font-medium">
            标题
          </label>
          <input
